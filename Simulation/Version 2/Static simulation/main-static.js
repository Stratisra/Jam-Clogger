...
(function() {
	...
	(function timer() {
		...
		// ************* Display next State ********************
			...
			//Horizontal road
			H_line(-10,screenh/2-200,screenw/2-90,screenh/2-200,5);
			H_line(screenw/2+15,screenh/2-200,2000,screenh/2-200,5);

			H_line(-10,screenh/2-100,screenw/2-90,screenh/2-100,2);
			H_line(screenw/2+15,screenh/2-100,2000,screenh/2-100,2);

			H_line(-10,screenh/2,screenw/2-90,screenh/2,5);
			H_line(screenw/2+15,screenh/2,2000,screenh/2,5);

			//Vertical road
			V_line(screenw/2-90,10,screenw/2-90,screenh/2-200,3);
			V_line(screenw/2+15,10,screenw/2+15,screenh/2-200,3);

			V_line(screenw/2-90,screenh/2,screenw/2-90,1280,3);
			V_line(screenw/2+15,screenh/2,screenw/2+15,1280,3);
	   
			const State = Traffic_States[s];
			
			//Traffic_Light & arrows 1
			if (State.transition>=0 && State.transition <=9 ){
				Traffic_Light(1, screenw/2-105,screenh/2+20, "lightgreen"); 
				ctx.drawImage(arrow_1_1, screenw/2-100, screenh/2-20, 40,40);
				ctx.drawImage(arrow_1_2, screenw/2-100, screenh/2-50, 50,50);
			} //light-green or red
			else {Traffic_Light(1, screenw/2-105,screenh/2+20, "red");}

			//Traffic_Light & arrows 2
			if (State.transition>=5 && State.transition <=9 || State.transition>=15 && State.transition <=19){
				Traffic_Light(2, screenw/2-105,screenh/2-100, "lightgreen");
				ctx.drawImage(arrow_2_1, screenw/2-100, screenh/2-100, 40,40);
				} //light-green or red
			else {Traffic_Light(2, screenw/2-105,screenh/2-100, "red");}
			
			//Traffic_Light & arrows 3
			if (State.transition>=0 && State.transition <=4 || State.transition>=10 && State.transition <=14){
				Traffic_Light(3, screenw/2+30,screenh/2-215, "lightgreen");
				ctx.drawImage(arrow_3_1, screenw/2-20, screenh/2-215, 40,40);
				ctx.drawImage(arrow_3_2, screenw/2-30, screenh/2-195, 50,50);
				} //light-green or red
			else {Traffic_Light(3, screenw/2+30,screenh/2-215, "red");}
			
			//Traffic_Light & arrows 4
			if (State.transition>=10 && State.transition <=19 ){
				Traffic_Light(4, screenw/2+30,screenh/2-100, "lightgreen");
				ctx.drawImage(arrow_4_1, screenw/2-20, screenh/2-140, 40,40);
				} //light-green or red
			else {Traffic_Light(4, screenw/2+30,screenh/2-100, "red");}
