...
class vehicleGenerator():
    ...
    def addVehicle(self):
        ...
        if random.random() > self.behaviour:
            self.vehicles += random.randint(1, 5)
...
def updateState(action):
    if action == 0:
        trafficLights[0].vehicles = trafficLights[0].vehicles - 10
        trafficLights[2].vehicles = trafficLights[2].vehicles - 10
    if action == 1:
        trafficLights[0].vehicles = trafficLights[0].vehicles - 10
        trafficLights[1].vehicles = trafficLights[1].vehicles - 5
    if action == 2:
        trafficLights[2].vehicles = trafficLights[2].vehicles - 10
        trafficLights[3].vehicles = trafficLights[3].vehicles - 5
    if action == 3:
        trafficLights[1].vehicles = trafficLights[1].vehicles - 5
        trafficLights[3].vehicles = trafficLights[3].vehicles - 5
    for i in range(4):
